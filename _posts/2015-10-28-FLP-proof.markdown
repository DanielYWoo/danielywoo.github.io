...
如果把e用于Ɛ中某一个configuration, 那么相当于Ɛ中离起始的C越远的configuration收到e越晚, e越被延迟. 由于我们定义了Ɛ不含e, 所以e只会发生在Ɛ的边界上, 实际上就是把e延迟成为离开Ɛ进入Ɗ的最后一个消息. e会把Ɛ内每一个configuration映射到Ɗ 中一个configuration.也就是说，我们可以推断出，从C到Ɗ之间，接收了两个事件；其中第二个事件就是e。
...
如果E0还没有收到e, 那么E0属于Ɛ, 那么E0收到e之后会变成F0. F0明显是0-valent（E0是0-valent的，因为参考lemma 2的证明，如果F0变为1-valent的话，则只存在一种可能即事件e的消息是1，假设e事件接受消息的进程p故障，则F0可能变为bivalent。但是Ɗ不包含bivalent configuration，故F0只可能是0-valent。综上所述，我们可以推出若configuration是n-valent的，那么其在接收事件e以后的子configuration也一定是n-valent的，即决策值已经确定。）并且属于Ɗ. 同样, 如果E1还没有收到e, 也会有F1属于Ɗ.
